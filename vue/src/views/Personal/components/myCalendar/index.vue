...
const goToDate = () => {
  // Sat Jan 25 2025 00:00:00 GMT+0800 (中国标准时间)
  if (!pickDate.value) {
    ElMessage({
      type: 'warning',
      message: '请选择一个日期'
  ...
  if (calendarApi) {
    calendarApi.gotoDate(pickDate.value); // 跳转到指定日期
  } else {
    console.error("FullCalendar 实例未就绪");
  }
}
...
const refreshDate = () => {
  const calendarApi = calendarRef.value?.getApi();
  if (calendarApi) {
    calendarApi.gotoDate(new Date()); // 跳转到指定日期
  } else {
    console.error("FullCalendar 实例未就绪");
  }
}
