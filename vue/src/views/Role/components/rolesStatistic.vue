  ...
  <div>
    <el-card shadow="hover" class="card">
      ...
      <div class="department">
        <h3>按部门统计</h3>
        <div class="e-chart" style="height: 201px; width: 100%">
          <div ref="refDepartmentsChart" style="width: inherit; height: inherit;"></div>
        </div>
        <div class="chart-widget-list">
          <p v-for="(count, department) in departmentsCount" :key="department">
            <span><i :class="getDepartmentIconClass(department)"></i> {{ department }} </span>
            <span>{{ count }}</span>
          </p>
        </div>
      </div>
...
interface stateTypes {
  ...
  departmentsCount: { [key: string]: number }
}
export default defineComponent({
  ...
  setup(props) {
    ...
    const refDepartmentsChart = ref<HTMLElement | null>(null)
    ...
    const departmentIconMapping = {
      "技术部": "icon-square red",
      "人力资源部": "icon-square green",
      "市场部": "icon-square deep-blue"
    }
    // 计算属性：总人数
    ...
    const getDepartmentIconClass = (department: string): string => {
      return departmentIconMapping[department] || "icon-square";
    }
    // 统计函数
    const doStatistics = () => {
      // 遍历数据进行统计
      state.data.forEach(record => {
        const role = roleMapping[record.userRole]|| record.userRole;
        if (role) {
      ...
      state.data.forEach(record => {
        const department = departmentMapping[record.userDepartment]|| record.userDepartment;
        if (department) {
          if (state.departmentsCount[department]) {
            state.departmentsCount[department]++;
          } else {
            state.departmentsCount[department] = 1;
          }
        }
      });
    };
    ...
    const displayChart = () => {
      ...
      // 部门统计图表数据
      const departmentsChartData = Object.entries(state.departmentsCount).map(([department, count]) => ({
        name: department,
        value: count
      }));
      ...
      nextTick(() => {
        ...
        if (refDepartmentsChart.value) {
          useInitPieChart(refDepartmentsChart.value, departmentsChartData);
        } else {
          console.log("refDepartmentsChart not exist!");
        }
      });
    };
    // 初始化统计与图表显示
